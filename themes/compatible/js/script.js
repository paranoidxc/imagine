...
var swfu=null;
function clear_swfu(){ if( swfu != null ) { swfu.destroy(); } }
...
$(document).ready(function(){
  ...
  if($.browser.msie && $.browser.version =="6.0") {
    if( $('#w_panel').length ) {
      if( $('#w_panel').height() < $(window).height() ) {
        $('#w_panel').css({'height': $(window).height()-32});
      }
    }
  } else { 
    $('#w_content').addClass('w_content_position');
    resize_layout(true);
  ...
  if( $('textarea.tinymce').length > 0 ) {
    $('textarea.tinymce').tinymce({
      ...
      theme : "advanced",
        ...
        fullscreen_settings : {
          setup: function(ed) {
            if( $('#mce_fullscreen_container').length > 0 ) {
